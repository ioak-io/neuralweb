...
const EditBillPage = (props: Props) => {
  ...
  const getEmptyBill = (): ReceiptModel => {
    return {
      ...EMPTY_BILL,
      billDate:
        sessionStorage.getItem(NEURALWEB_PREF_ADDBILL_DATE) ||
        EMPTY_BILL.billDate,
    };
  };
  ...
  useEffect(() => {
    if (sessionStorage.getItem(NEURALWEB_PREF_ADDBILL_ANOTHER)) {
      setAddAnother(
        sessionStorage.getItem(NEURALWEB_PREF_ADDBILL_ANOTHER) === 'true'
      );
    }
  }, []);
